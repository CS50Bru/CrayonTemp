...
namespace detail
{
	template<length_t L, typename ComponentType, qualifier Q, bool compute = false>
	struct compute_ceilShift
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call(vec<L, ComponentType, Q> const& v, ComponentType)
		{
			return v;
		}
	};
	...
	struct compute_ceilShift<L, ComponentType, Q, true>
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call(vec<L, ComponentType, Q> const& v, ComponentType Shift)
		{
			return v | (v >> Shift);
		}
	};

	template<length_t L, typename ComponentType, qualifier Q, bool isSigned = true>
	struct compute_ceilPowerOfTwo
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call(vec<L, ComponentType, Q> const& x)
		{
			GLM_STATIC_ASSERT(!std::numeric_limits<ComponentType>::is_iec559, "'ceilPowerOfTwo' only accept integer scalar or vector inputs");

			vec<L, ComponentType, Q> const Sign(sign(x));

			vec<L, ComponentType, Q> v(abs(x));

			v = v - static_cast<ComponentType>(1);
			v = v | (v >> static_cast<ComponentType>(1));
			v = v | (v >> static_cast<ComponentType>(2));
			v = v | (v >> static_cast<ComponentType>(4));
			v = compute_ceilShift<L, ComponentType, Q, sizeof(ComponentType) >= 2>::call(v, 8);
			v = compute_ceilShift<L, ComponentType, Q, sizeof(ComponentType) >= 4>::call(v, 16);
			v = compute_ceilShift<L, ComponentType, Q, sizeof(ComponentType) >= 8>::call(v, 32);
			return (v + static_cast<ComponentType>(1)) * Sign;
		}
	};

	template<length_t L, typename ComponentType, qualifier Q>
	struct compute_ceilPowerOfTwo<L, ComponentType, Q, false>
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call(vec<L, ComponentType, Q> const& x)
		{
			GLM_STATIC_ASSERT(!std::numeric_limits<ComponentType>::is_iec559, "'ceilPowerOfTwo' only accept integer scalar or vector inputs");

			vec<L, ComponentType, Q> v(x);
			...
			v = compute_ceilShift<L, ComponentType, Q, sizeof(ComponentType) >= 4>::call(v, 16);
			v = compute_ceilShift<L, ComponentType, Q, sizeof(ComponentType) >= 8>::call(v, 32);
			return v + static_cast<ComponentType>(1);
		}
	};
