...
namespace detail
{
	...
	struct compute_rand
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call();
	};
	...
	{
		GLM_FUNC_QUALIFIER static vec<L, ComponentType, Q> call(vec<L, ComponentType, Q> const& Min, vec<L, ComponentType, Q> const& Max);
	};
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> linearRand(vec<L, ComponentType, Q> const& Min, vec<L, ComponentType, Q> const& Max)
	{
		return detail::compute_linearRand<L, ComponentType, Q>::call(Min, Max);
	}
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> gaussRand(vec<L, ComponentType, Q> const& Mean, vec<L, ComponentType, Q> const& Deviation)
	{
		return detail::functor2<vec, L, ComponentType, Q>::call(gaussRand, Mean, Deviation);
	}

	template<typename ComponentType>
	GLM_FUNC_QUALIFIER vec<2, ComponentType, defaultp> diskRand(ComponentType Radius)
	{
		assert(Radius > static_cast<ComponentType>(0));

		vec<2, ComponentType, defaultp> Result(ComponentType(0));
		ComponentType LenRadius(ComponentType(0));

		do
		{
			Result = linearRand(
				vec<2, ComponentType, defaultp>(-Radius),
				vec<2, ComponentType, defaultp>(Radius));
	...
	template<typename ComponentType>
	GLM_FUNC_QUALIFIER vec<3, ComponentType, defaultp> ballRand(ComponentType Radius)
	{
		assert(Radius > static_cast<ComponentType>(0));

		vec<3, ComponentType, defaultp> Result(ComponentType(0));
		ComponentType LenRadius(ComponentType(0));

		do
		{
			Result = linearRand(
				vec<3, ComponentType, defaultp>(-Radius),
				vec<3, ComponentType, defaultp>(Radius));
			LenRadius = length(Result);
		}
		while(LenRadius > Radius);
	...
	template<typename ComponentType>
	GLM_FUNC_QUALIFIER vec<2, ComponentType, defaultp> circularRand(ComponentType Radius)
	{
		assert(Radius > static_cast<ComponentType>(0));

		ComponentType a = linearRand(ComponentType(0), static_cast<ComponentType>(6.283185307179586476925286766559));
		return vec<2, ComponentType, defaultp>(glm::cos(a), glm::sin(a)) * Radius;
	}

	template<typename ComponentType>
	GLM_FUNC_QUALIFIER vec<3, ComponentType, defaultp> sphericalRand(ComponentType Radius)
	{
		assert(Radius > static_cast<ComponentType>(0));

		ComponentType theta = linearRand(ComponentType(0), ComponentType(6.283185307179586476925286766559f));
		ComponentType phi = std::acos(linearRand(ComponentType(-1.0f), ComponentType(1.0f)));

		ComponentType x = std::sin(phi) * std::cos(theta);
		ComponentType y = std::sin(phi) * std::sin(theta);
		ComponentType z = std::cos(phi);

		return vec<3, ComponentType, defaultp>(x, y, z) * Radius;
	}
}//namespace glm
