...
//	}
	...
	GLM_FUNC_QUALIFIER vec<4, ComponentType, Q> packRGBM(vec<3, ComponentType, Q> const& rgb)
	{
		vec<3, ComponentType, Q> const Color(rgb * static_cast<ComponentType>(1.0 / 6.0));
		ComponentType Alpha = clamp(max(max(Color.x, Color.y), max(Color.z, static_cast<ComponentType>(1e-6))), static_cast<ComponentType>(0), static_cast<ComponentType>(1));
		Alpha = ceil(Alpha * static_cast<ComponentType>(255.0)) / static_cast<ComponentType>(255.0);
		return vec<4, ComponentType, Q>(Color / Alpha, Alpha);
	}

	template<typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<3, ComponentType, Q> unpackRGBM(vec<4, ComponentType, Q> const& rgbm)
	{
		return vec<3, ComponentType, Q>(rgbm.x, rgbm.y, rgbm.z) * rgbm.w * static_cast<ComponentType>(6);
	}
