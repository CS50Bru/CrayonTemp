...
namespace detail
{
	...
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> mask(vec<L, ComponentType, Q> const& v)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<ComponentType>::is_integer, "'mask' accepts only integer values");

		return detail::functor1<vec, L, ComponentType, ComponentType, Q>::call(mask, v);
	}
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> bitfieldRotateRight(vec<L, ComponentType, Q> const& In, int Shift)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<ComponentType>::is_integer, "'bitfieldRotateRight' accepts only integer values");

		int const BitSize = static_cast<int>(sizeof(ComponentType) * 8);
		return (In << static_cast<ComponentType>(Shift)) | (In >> static_cast<ComponentType>(BitSize - Shift));
	}
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> bitfieldRotateLeft(vec<L, ComponentType, Q> const& In, int Shift)
	{
		GLM_STATIC_ASSERT(std::numeric_limits<ComponentType>::is_integer, "'bitfieldRotateLeft' accepts only integer values");
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> bitfieldFillOne(vec<L, ComponentType, Q> const& Value, int FirstBit, int BitCount)
	{
		return Value | static_cast<ComponentType>(mask(BitCount) << FirstBit);
	}
	...
	template<length_t L, typename ComponentType, qualifier Q>
	GLM_FUNC_QUALIFIER vec<L, ComponentType, Q> bitfieldFillZero(vec<L, ComponentType, Q> const& Value, int FirstBit, int BitCount)
	{
		return Value & static_cast<ComponentType>(~(mask(BitCount) << FirstBit));
	}
