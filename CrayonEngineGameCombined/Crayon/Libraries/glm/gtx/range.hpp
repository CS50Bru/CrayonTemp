...
#	endif
	...
	inline length_t components(vec<1, ComponentType, Q> const& v)
	{
		return v.length();
	}

	template<typename ComponentType, qualifier Q>
	inline length_t components(vec<2, ComponentType, Q> const& v)
	{
		return v.length();
	}

	template<typename ComponentType, qualifier Q>
	inline length_t components(vec<3, ComponentType, Q> const& v)
	{
		return v.length();
	}

	template<typename ComponentType, qualifier Q>
	inline length_t components(vec<4, ComponentType, Q> const& v)
	{
		return v.length();
	}
